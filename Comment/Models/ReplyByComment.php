...
class ReplyByComment
{
	...
	public function insert()
	{
		...
		$prepareStatement = $this->_session->prepare(
			"INSERT INTO reply_by_comment (article_id, parent_id, reply_id, reply_text, username, user_id, user_avatar, user_full_name)
	...
	public static function findByOwner($session, $article_id, $parent_id)
	{
		$prepareStatement = $session->prepare(
			"SELECT JSON article_id, parent_id, reply_id, unixTimestampOf(reply_id) AS datetime_reply, reply_text, username, user_id, user_avatar, user_full_name FROM reply_by_comment WHERE article_id = :article_id AND parent_id = :parent_id ORDER BY parent_id DESC"
		);
	...
	{
		$prepareStatement = $session->prepare(
			"SELECT article_id, parent_id, reply_id, unixTimestampOf(reply_id) AS datetime_reply, reply_text, username, user_id, user_avatar, user_full_name FROM reply_by_comment"
		);
	...
	{
		$prepareStatement = $session->prepare(
			"SELECT * FROM reply_by_comment WHERE article_id = :article_id AND parent_id = :parent_id AND reply_id = :reply_id"
		);
	...
	{
		$prepareStatement = $this->_session->prepare(
			"DELETE FROM reply_by_comment WHERE article_id = :article_id AND reply_id = :reply_id AND parent_id = :parent_id"
		);
	...
	{
		$prepareStatement = $this->_session->prepare(
			"DELETE FROM reply_by_comment WHERE article_id = :article_id AND parent_id = :parent_id"
		);
	...
	{
		$prepareStatement = $this->_session->prepare(
			"UPDATE reply_by_comment SET reply_text = :reply_text
